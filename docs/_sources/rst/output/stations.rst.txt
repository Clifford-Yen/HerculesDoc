...
When ``number_output_stations`` is set to a positive integer, Hercules will generate responses at the stations (observation points) specified by the parameter ``output_stations``. For definition of the related parameters, see the section :ref:`Output for stations`.
...
Note that the time step in ``station.X`` files is defined by the parameter ``output_stations_print_rate`` and ``simulation_delta_time_sec``.